...
export async function scheduleRoutes(app) {
    ...
    app.post('/schedules', { preHandler: [authenticate, idempotencyPreHandler('schedules.create')] }, async (req, reply) => {
        ...
        if (participants.length > 0) {
            ...
            const pickProjectIdForUser = (uid) => {
                const userProjects = projectsByUser.get(uid) ?? [];
                if (schedule.projectId && userProjects.includes(schedule.projectId))
            ...
            for (const p of participants) {
                const pid = pickProjectIdForUser(p.userId);
                if (pid)
    ...
    // ─── GET /schedules ───────────────────────────────────────
    app.get('/schedules', { preHandler: authenticate }, async (req, _reply) => {
    ...
    // ─── GET /schedules/:id ───────────────────────────────────
    app.get('/schedules/:id', { preHandler: authenticate }, async (req, reply) => {
        const { id } = req.params;
        const userId = req.authUser.id;
    ...
    // ─── PATCH /schedules/:id ─────────────────────────────────
    app.patch('/schedules/:id', { preHandler: authenticate }, async (req, reply) => {
        const { id } = req.params;
        const user = req.authUser;
        ...
        if (body.attendees !== undefined) {
            ...
            const existingResponseByEmail = new Map(existing.attendees.map(a => [a.email, a.response]));
    ...
    app.patch('/schedules/:id/respond', { preHandler: authenticate }, async (req, reply) => {
        const { id } = req.params;
        const user = req.authUser;
        ...
        if (schedule) {
            ...
            if (participants.length > 0) {
                ...
                const responseType = body.response === ScheduleAttendeeResponse.ACCEPTED ? 'SCHEDULE_INVITE_ACCEPTED' : 'SCHEDULE_INVITE_DECLINED';
    ...
    // ─── DELETE /schedules/:id ────────────────────────────────
    app.delete('/schedules/:id', { preHandler: authenticate }, async (req, reply) => {
        const { id } = req.params;
        const user = req.authUser;
        const schedule = await prisma.schedule.findUnique({
            where: { id },
    ...
    app.get('/projects/:projectId/schedules', { preHandler: authenticate }, async (req, reply) => {
