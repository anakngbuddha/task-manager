...
export async function adminRoutes(app) {
    ...
    app.get('/admin/metrics', async (req, reply) => {
        ...
        const [totalUsers, activeProjects, totalTasks, recentSignups, completedTasks, overdueTasks, totalInvites, acceptedInvites, totalNotifications, readNotifications, dauCount, wauCount, churnRiskUsers,] = await Promise.all([
            prisma.user.count(),
        ...
        const avgCompletionDays = completedTasksSample.length === 0
            ? 0
            : Math.round(completedTasksSample.reduce((acc, t) => {
    ...
    app.get('/admin/analytics/extended', async (req, reply) => {
        ...
        const avgTasksPerUser = totalMembers === 0 ? 0 : Math.round((totalTaskCount / totalMembers) * 10) / 10;
        ...
        const subtaskUsageRate = totalTaskCountAll === 0 ? 0 : Math.round((subtaskCount / totalTaskCountAll) * 100);
    ...
    app.post('/admin/users/ai-tester', async (req, reply) => {
        ...
        try {
            await prisma.$transaction(async (tx) => {
                const testers = await tx.$queryRaw `
