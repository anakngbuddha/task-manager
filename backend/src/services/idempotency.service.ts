...
declare module 'fastify' {
  interface FastifyRequest {
    idempotencyCtx?: { recordId: string; finished: boolean }
  }
}
...
export function hashForIdempotency(routeKey: string, req: FastifyRequest): string {
  const payload = {
    body: req.body ?? null,
    query: req.query ?? {},
    params: req.params ?? {},
...
function readIdempotencyHeader(req: FastifyRequest): string | null {
...
export async function acquireIdempotency(
  ...
  const userId = (req as FastifyRequest & { authUser?: { id: string } }).authUser?.id
  if (!userId) {
...
export function attachIdempotencyContext(req: FastifyRequest, recordId: string) {
  if (!recordId) return
  req.idempotencyCtx = {
    recordId,
    finished: false,
...
export async function completeIdempotencyFromPayload(
  req: FastifyRequest,
  reply: FastifyReply,
  payload: unknown,
): Promise<unknown> {
  const ctx = req.idempotencyCtx
  if (!ctx?.recordId || ctx.finished) return payload
  ctx.finished = true
  ...
  if (typeof payload === 'string' && payload.length > 0) {
    ...
    try {
      body = JSON.parse(payload.toString('utf8')) as unknown
    } catch {
  ...
  try {
    await prisma.idempotencyKey.update({
      ...
      data: {
        status: 'COMPLETED',
        responseCode: code,
        responseBody: body === null || body === undefined ? Prisma.JsonNull : (body as Prisma.InputJsonValue),
      },
    })
  } catch (err) {
