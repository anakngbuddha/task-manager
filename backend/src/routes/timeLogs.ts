...
export async function timeLogRoutes(app: FastifyInstance) {
  ...
  app.delete(
    ...
    async (req, reply) => {
      ...
      if (!log) return reply.status(404).send({ error: 'Time log not found' })

      const projectId = log.task?.projectId
      if (!projectId) return reply.status(404).send({ error: 'Project not found' })
      ...
      if (!isElevated && !isOwner) return reply.status(403).send({ error: 'Forbidden' })
