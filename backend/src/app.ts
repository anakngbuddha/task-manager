...
function setSecurityHeaders(res: any, csp: string) {
...
const authLimits: { test: (url: string) => boolean; max: number; windowMs: number }[] = [
...
setInterval(() => {
  const now = Date.now()
  for (const [k, v] of authBuckets) if (v.resetAt < now) authBuckets.delete(k)
}, 60_000).unref?.()

async function checkAuthRateLimit(ip: string, url: string): Promise<{ allowed: boolean; retryAfter: number }> {
  ...
  if (redisAvailable) {
    try {
      ...
      if (count === 1) await redis.expire(key, windowExpiresSec)
      if (count > limit.max) {
...
app.setErrorHandler((err, req, reply) => {
  ...
  const status = (err as any).statusCode && Number.isInteger((err as any).statusCode)
